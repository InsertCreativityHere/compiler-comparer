...
if len(sys.argv) < 2:
    print("This command should be run like:);
    print("compiler-comparer.py <old-branch> <new-branch> [compilers...]");
    print();
    print("For example:");
    print("compiler-comparer.py main my-feature slice2cpp slice2java");
    sys.exit(79);
oldBranch = sys.argv[0];
newBranch = sys.argv[1];
compilers = sys.argv[2:];
if DEBUGGING:
    print("oldBranch = '" + str(oldBranch) + "'");
    print("newBranch = '" + str(newBranch) + "'");
    print("compilers = '" + str(compilers) + "'");



# Second, make sure we can locate a project file for msbuild to work with.
if os.path.exists("ice.proj"):
    projPath = "ice.proj;
elif os.path.exists("msbuild\\ice.proj"):
    projPath = "msbuild\\ice.proj";
else:
    # TODO write a better check for finding project files to build.
    print("Failed to locate project file for building the thing");
    sys.exit(80);
if DEBUGGING:
    print("projPath = '" + str(projPath) + "'");
...
# Third, recursively find all the Slice in the current directory.
currentDir = os.getcwd();
globPattern = os.path.join(currentDir, "**/*.ice");
sliceFiles = [file for file in glob.iglob(globPattern, recursive=True)];
print("Detected a total of " + str(len(sliceFiles)) + " Slice files");
if DEBUGGING:
...
# Remove the 'cwd' from them; we want paths that are relative to 'cwd'.
wereThereMessedUpFiles = False;
for file in sliceFiles:
    if (file.find(currentDir) != 0):
        print("This file is outside of the current working directory somehow?? : '" + file + "'");
        wereThereMessedUpFiles = True;
if wereThereMessedUpFiles:
    sys.exit(81);
cwdLength = len(currentDir);
sliceFiles = [file[cwdLength:] for file in sliceFiles];
if DEBUGGING:
    print()
    print("(relative)sliceFiles = '" + str(sliceFiles) + "'");
    print()



# Define a function for checking out and building a particular branch.
def checkout_and_build(branchName):
    subprocess.run(["git", "checkout", branchName], check=True);
    subprocess.run(["git", "clean", "-dfx], check=True);
    subprocess.run(["msbuild", projPath, "/p:Configuration=Debug", "/p:Platform=x64", "/p:PythonHome=\"C:\\Program Files\\Python312\"", "/m", "/nr:false"], check=True);

# And another function for running the Slice compilers.
def compile_the_slice_files(compiler):
...
# Let's get this party started by building the old branch.
checkout_and_build(oldBranch);
TODOTODOTODO
...
# We want to stash the compiler output before we clean everything!!
checkout_and_build(newBranch);
'''